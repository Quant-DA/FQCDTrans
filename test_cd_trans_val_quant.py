...
project_name = 'PSAQ_FQCDTrans_val4cal_8/8/4'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", default='cuda', help="GPU device", type=str)
    ...
    if args.quant:
        ...
        with torch.no_grad():
            # for i, (image, vid, _, _, _, idx) in enumerate(train_loader):
            # batch_random = np.random.randint(31, size=1)
            # log(f'Batch Number: {batch_random}')
            for i, (image, _, _, _, _, _) in enumerate(val_loader): # img: [64, 3, 224, 224] -> train loader batch: 32 -> Just use one batch
                # if i == batch_random:
                if i ==
