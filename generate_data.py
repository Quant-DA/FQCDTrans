...
def generate_data(args, model=None, name=None):
    ...
    if model is None:
        ...
        p_model = model
    # p_model.fc = nn.Linear(p_model.embed_dim, args.n_classes) if args.n_classes > 0 else nn.Identity()
    ...
    for m in p_model.blocks:
        hooks.append(AttentionMap(m.attn.matmul2)) # 11 blocks, None in this phase
    ...
    # Set pseudo labels
    # Need to change pseudo label according to number of classes in the dataset
    pred = torch.LongTensor([random.randint(0, args.n_classes-1) for _ in range(args.batch_size)]).to('cuda')
    var_pred = random.uniform(2500, 3000)  # for batch_size 32

    criterion = nn.CrossEntropyLoss()

    # Train for two epochs
    for lr_it in range(args.epochs):
        if lr_it == 0:
        ...
        with tqdm(range(iterations_per_layer)) as pbar:
            for itr in pbar:
                pbar.set_description(f"Epochs {lr_it+1}/{args.epochs}")
                ...
                if name is 'CDTrans': 
                    _, output, _ = p_model(None, img_jit, target_branch_only=True) # output: [batch, n_class], Type: Tensor
                else:
                    output = p_model(img_jit)

                if isinstance(output, tuple):
                    output = torch.tensor(output) # For using CDTrans
                ...
                for itr_hook in range(len(hooks)):
                    # Hook attention
                    # print(hooks[itr_hook].feature) # should have 11 tensors 
