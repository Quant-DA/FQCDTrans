...
def do_inference_uda(cfg,
    ...
    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
            with torch.no_grad():
                ...
                if cfg.MODEL.TASK_TYPE == 'classify_DA':
                    probs = model(None, img, target_branch_only=True)
    ...
    accuracy, mean_ent = evaluator.compute() 
    # logger("Classify Domain Adapatation Validation Results - In the source trained model")
    # logger("Accuracy: {:.1%}".format(accuracy))
